...
class Child_Theme
{
    ...
    /**
     * Get the current branding settings, merged with defaults.
     *
     * @return array<string, string> Branding values.
     */
    public static function get_branding(): array
    {
        $saved = get_option(Constants::OPTION_BRANDING, []);

        if (!is_array($saved)) {
            $saved = [];
        }

        return array_merge(Constants::DEFAULT_BRANDING, $saved);
    }
    ...
    private static function write_root_style(string $child_dir): bool
    {
        ...
        $theme_header = apply_filters('epb_child_theme_header', [
            ...
            'uri'         => $branding['company_url'],
            'description' => $branding['description'],
            'author'      => $branding['company_name'],
            'author_uri'  => $branding['company_url'],
            'template'    => 'yootheme',
    ...
    private static function write_functions_php(string $child_dir, bool $force = false): bool
    {
        ...
        $branding = self::get_branding();
        $filtered = apply_filters('epb_child_theme_branding', [
            'company_name' => $branding['company_name'],
            'company_url'  => $branding['company_url'],
            'logo_url'     => $branding['logo_url'],
        ]);

        $company_name = esc_attr($filtered['company_name']);
        $company_url  = esc_url($filtered['company_url']);
        $logo_url     = esc_url($filtered['logo_url']);
 ...
 */
function ct_child_scripts()
{
    // Enqueue Parent Stylesheet
    wp_enqueue_style('ct-parent-stylesheet', get_template_directory_uri() . '/style.css');
    // Adobe Fonts are managed via the Enhanced Plugin Bundle admin panel.
    wp_enqueue_style('ct-child-theme', get_stylesheet_directory_uri() . '/style.css', array('ct-parent-stylesheet'));
}
add_action('wp_enqueue_scripts', 'ct_child_scripts');
...
PHP;
     ...
     * @var string
     */
    public const LESS_STYLE_NAME = 'ct-style';
    ...
    public static function generate_less_style(): string
    {
        ...
        // Add @font-face rules for custom fonts (valid CSS inside Less).
        $font_face = Custom_Font::generate_font_face_css();
        if (!empty($font_face)) {
            $less .= "// ==========================================================================\n";
            $less .= "//   Custom Font Faces\n";
            $less .= "//   Generated by Enhanced Plugin Bundle\n";
            $less .= "// ==========================================================================\n\n";
            $less .= $font_face;
        }
...
<?php
/**
 * YOOtheme Pro module configuration for CT Child Theme.
 *
