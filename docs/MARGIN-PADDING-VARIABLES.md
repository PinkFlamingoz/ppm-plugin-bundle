...
> **Note:** UIkit also uses `@global-gutter`, `@global-small-gutter`, `@global-medium-gutter`, and `@global-large-gutter` for horizontal spacing (gutters), which many padding variables reference. These are separate from margin variables but equally important for spacing.
